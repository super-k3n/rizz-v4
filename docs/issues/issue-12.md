...
- [ ] 期間別データ集計APIの実装
  - [ ] 日次データの集計クエリ
    - [ ] 実績値：`daily_records`テーブルから日付ごとに集計
    - [ ] 目標値：`daily_goals`テーブルから日付ごとに取得
  - [ ] 週次データの集計クエリ
    - [ ] 実績値：`daily_records`テーブルから週ごとに集計
    - [ ] 目標値：`goals`テーブルから週次（weekly）データを取得
  - [ ] 月次データの集計クエリ
    - [ ] 実績値：`daily_records`テーブルから月ごとに集計
    - [ ] 目標値：`goals`テーブルから月次（monthly）データを取得
  - [ ] 年次データの集計クエリ
    - [ ] 実績値：`daily_records`テーブルから年ごとに集計
    - [ ] 目標値：`goals`テーブルから年次（yearly）データを取得
- [ ] パフォーマンス最適化
  - [ ] インデックスの設定（日付カラムの最適化）
  - [ ] キャッシュ戦略の検討
  - [ ] クエリの効率化
